...
NewsSaveService wxggs = (NewsSaveService)factory.getBean("NewsSaveService");
List<Newssave> wxgglist = wxggs.findByFbbzType(false,1);                    //数据库中筛选数据 
%>
				...
				<div class="block-inner">
					...
					<table border = "1px"> 
						...
						<tbody id="itemContainer">
							<% 
							for(int i = 0; i <wxgglist.size(); i++){ 
								Newssave tmp = wxgglist.get(i); 
							%> 
							<tr> 
							<td class = "td content">
								<a href = <%="htgl/ggxq.jsp?id=" + tmp.getId() %> rel="external">
